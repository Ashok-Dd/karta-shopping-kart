...
async function getProducts(params: SearchParams) {
  ...
  const orderBy: Record<string, string> =
    ...
    : params.sort === "popular"    ? { reviewCount: "desc" }
    : { createdAt: "desc" };
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-10">
            ...
            >
              ...
              {isSearch ? (
                <span className="flex items-center gap-1">
                  Clear search
                  <span className="flex items-center justify-center w-4 h-4 rounded-full" style={{ background: "var(--color-surface-2)" }}>
                    <X size={10} />
                  </span>
                </span>
              ) : "All Products"}
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <aside className="w-full lg:w-56 shrink-0">
            <Suspense fallback={<div className="animate-shimmer h-64 rounded-2xl" />}>
          ...
          <div className="flex-1 min-w-0">
            {products.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-24 text-center">
                ...
                <h3 className="font-display text-xl mb-2" style={{ color: "var(--color-text)" }}>
                  No products found
                </h3>
                <p style={{ color: "var(--color-text-muted)", fontSize: "0.875rem" }} className="mb-6">
                  {isSearch
                  ...
                  <div className="flex items-center justify-center gap-2 mt-12">
                    {Array.from({ length: totalPages }, (_, i) => i + 1).map((p) => (
                      <a
                        ...
                        style={{
                          background:  p === page ? "var(--color-accent-2)" : "var(--color-surface)",
